...
class MyWindow(QtWidgets.QMainWindow):
    ...
    def ATP_files_execution(self):
        ...
        with open("Lista de fallas\\FileListATPFault.txt", "r") as f:
            intime = time()
            ext = ("*.bin", "*.tmp", "SCENARIOS_ATP\\*.dbg", "SCENARIOS_ATP\\*.lis")
            for linea in f:
                atp_file = linea.rstrip("\n")
                self.pl4_file = atp_file.replace(".atp", ".pl4")

                print("Scenario " + atp_file + " Started")
                atp_file_path = currentDirectory + "\\SCENARIOS_ATP"
                ...
                self.readPL4(self.pl4_file)
                # self.pl4_to_csv(self.pl4_file)
                ...
                for file in files_to_delete:
                    os.remove(file)
                # lista_archivos = os.listdir(currentDirectory)
                # archivos_filtrados = [
                #     file for file in lista_archivos if file.endswith(ext)
                # ]
                # for file in archivos_filtrados:
                #     PATH = os.path.join(currentDirectory, file)
                #     os.remove(PATH)
